...
public class InstrumentFieldResolver {
    ...
    public ResolvedInstrument resolve(String tickerOrIsin, String userName, TransactionType side) {
        ...
        if (OpenFigiIsinConverter.isIsin(tickerOrIsin)) {
            ...
            // already owns) means the ticker just came back from OpenFIGI — register it as a stock
            // with its Yahoo symbol now, since HoldingComputeService's later getOrCreate() only
            // sees the bare ticker string and can't tell it apart from a manual crypto entry.
            if (!OpenFigiIsinConverter.isTrCryptoIsin(tickerOrIsin)) {
