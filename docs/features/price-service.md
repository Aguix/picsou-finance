...
- **CoinGecko rate limits (circuit breaker)**: The free/anonymous tier rate-limits hard — a burst 429s within ~5 calls, and a single dashboard load prices ~17 coins across several concurrent endpoints (dashboard, history, pnl, /prices, holdings). To avoid hammering CoinGecko and flooding the logs, `CoinGeckoPriceProvider` opens a **circuit breaker** on the first `429`: it sets `pausedUntil = now + Retry-After` (observed ~60s, served by CoinGecko's Cloudflare edge; fallback `DEFAULT_RETRY_AFTER` = 60s when the header is absent/unparseable) and, while `now < pausedUntil`, **every** method short-circuits to an empty result with no HTTP call and no log spam (the pause is logged once). So a page load never blocks nor retries in a storm — it renders with whatever's cached, and the 15-minute cache + hourly scheduler + boot backfill refill once the window clears. Consequence: under throttling the historical backfill fills only the coins reached before the breaker trips; the rest fill on a later run — **a Demo key (100/min) is the real unlock** and avoids the breaker almost entirely. `Retry-After` is honoured but not contractually documented by CoinGecko, hence the fallback.
- **Backfill is gap-aware and portfolio-anchored**: `backfillHistoricalPrices` fetches only the missing tail per ticker — it reads the latest stored snapshot date and, if history already reaches yesterday, fetches *nothing* for that coin (today's price is the live path's job). So the boot-time `PriceBackfillRunner` is a no-op on a warm restart instead of re-downloading the full window and burning the rate limit; a fresh import (no snapshots) still gets the full deep history. Assumes contiguous history from `from` — interior gaps aren't detected, but the provider only ever saves whole ranges (429 → empty → nothing saved), so partial ranges don't occur. The boot window is **anchored per ticker** — `PriceBackfillRunner` fetches each coin from *its own* earliest transaction (`TransactionRepository.findEarliestDatesByTickerIn` → a `ticker → firstDate` map passed to `backfillHistoricalPrices`), so a coin bought last month isn't fetched from a coin bought two years ago; holdings with no transaction timeline fall back to 12 months. This is why the free-tier 365-day cap only truncates coins whose *own* history predates a year (e.g. those first bought >365 days ago), not recently-bought ones. (The crypto-import path passes a single first-transaction date, since those rows aren't persisted yet when its backfill runs.)
- **Free tier caps historical depth at 365 days**: CoinGecko's Public/Demo tier only serves `market_chart/range` for the past ~365 days — an older `from` returns **401** regardless of the window size (verified: a 20-day window entirely beyond 365 days still 401s, so it's the `from` age, not the range span; chunking doesn't help). `getHistoricalPricesEur` clamps `from` to `MAX_FREE_HISTORY_DAYS` (364) so it returns the most recent year instead of failing outright. Consequence: on the free/Demo tier a portfolio older than a year has its value curve truncated to the last 365 days — deeper history needs a paid Pro key. This interacts with the portfolio-anchored backfill: the anchor may point years back, but the provider silently caps it.
...
- **Resolving at preview costs `/search` calls**: `preview()` resolves every ticker up front (to flag `unresolvedTickers`), so a preview of an unseen CSV fans out one CoinGecko `/search` per new coin — the same work the import would do, just moved earlier. These `/search` calls honour the circuit breaker (see above), so on the anonymous tier a large first-time preview can trip it partway and report the remaining coins as `unresolved` (the operator can still supply the link, or a Demo key avoids the limit). Blocking the preview to wait out the limit would be worse UX than a re-runnable prompt.
