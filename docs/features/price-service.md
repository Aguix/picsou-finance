...
Since V80, ticker → aggregator-id resolution is **dynamic**: the `financial_asset` table holds one row per priceable asset (symbol, name, type, status, one nullable ref column per aggregator), replacing the hardcoded ticker registries the providers used to carry.
...
**Re-pricing a symbol also rebuilds the account value history that was computed from it.** `price_snapshot` is only the per-asset daily price; each account's daily net-worth curve is stored separately in `balance_snapshot` (`Σ heldQty × price` per day), so purging/refetching a symbol's prices would otherwise leave every holder's curve frozen on the *old* valuation. After a remap (`applyMappings`), a `clearMapping`, or a `markWorthless`, `FinancialAssetService` therefore rebuilds — through the shared **`BalanceHistoryService`** — the `balance_snapshot` rows of every account holding the symbol, sourced from the account's *stored* BUY/SELL/REWARD transactions and the now-corrected price history (the same reconstruction the crypto CSV import uses). Best-effort (a failure is logged, never rolls back the mapping change) and idempotent (rows are unique on account+date, updated in place). `delete` needs no rebuild: it only runs on an orphan symbol no account holds.

**The rebuild self-gates on the trade timeline, not the account type.** A `balance_snapshot` row is an account's *total* value, so the rebuild replays the whole account and overwrites its curve — which is only faithful when the account's value is entirely trade-derived (a CSV import, or manually-entered trades). A **balance-synced** account (bank/exchange/wallet, and a crypto ETP sitting in a mixed `COMPTE_TITRES` — cf. TR `XF000`) carries its positions as holdings with *no* BUY/SELL/REWARD rows; replaying it would drop those positions into a partial curve, worse than the stale valuation. So `rebuildFromTransactions` rebuilds an account **only when the replay reproduces its current holdings** quantity-for-quantity (`timelineReproducesHoldings`, mirroring `HoldingComputeService`) and leaves any other account untouched (logged). This is what lets `FinancialAssetService` hand *every* holder to the rebuild safely, regardless of `AccountType` — the account itself decides whether it can be faithfully replayed. **Known gap:** a balance-only account that legitimately holds the remapped coin (a wallet with no trade history) is *skipped* rather than rebuilt, so its past curve keeps the old valuation; reconstructing it would mean assuming today's quantity was held throughout (no purchase curve), a product call left open.

`markWorthless` pins a delisted coin to zero, re-values its live holdings, and rebuilds its holders' value history (as above) — its whole `price_snapshot` history is purged, so the symbol contributes zero across the rebuilt curve.
...
The import preview only surfaces a coin *while* importing. The **standing** counterpart is per-holding and always available: the crypto holding detail (`HoldingDetailModal`) carries an **Aggregator links** card (`AggregatorLinkCard`) that shows the symbol's current resolution status and which aggregators price it, and lets the operator verify or correct it any time — **one picker per aggregator** (same shape as the import wizard), each pre-filled with the id it holds today or its dominant suggestion, plus a paste-a-link escape hatch, mark-worthless, and forget. The account-detail holdings table badges each crypto row with its status (`AssetStatusBadge`), so an unresolved (`PENDING`) coin is visible at a glance without opening anything.
...
- `GET /api/assets/{symbol}/candidates` (any member) → the current status and **one block per aggregator** (`AssetCandidatesResponse.from(previewResolution)`), each carrying that aggregator's candidates, its market-cap suggestion, and the ref it holds today (`currentId`, so the editor pre-selects the current mapping). `previewResolution` is the single-symbol sibling of `previewResolutions` that, unlike it, returns candidates even for a coin already settled (so a standing mapping can always be re-verified) and reads each resolver's current ref off the registry row. Same generic shape as the import preview — the UI loops over the blocks, so a new aggregator needs no change. Fetched **lazily** by the card (only once the operator opens the editor) to respect the aggregators' rate limits.
- `PUT /api/assets/{symbol}/mapping` (admin) → apply a mapping: a pasted link (`setManualMapping`, resolved by whichever aggregator recognises it, taking precedence) or the ids picked per aggregator (`AssetMappingRequest.aggregatorIds → applyMappings`) land `USER`; `action=WORTHLESS` calls `markWorthless`.
- `DELETE /api/assets/{symbol}` (admin) → `clearMapping` — un-links the symbol (reverts it to `PENDING`, purges its price history, and rebuilds its holders' value history) but **keeps the registry row**, so a holding's `asset_id` FK stays valid. A full row `delete` would fail for any held symbol (the FK has no cascade), and the "forget the link" button always runs on a held coin, so it clears rather than deletes.

Two surfaces consume these. **Per-holding**: the `AggregatorLinkCard` appears both in the crypto holding detail (`HoldingDetailModal`, dashboard) and in the buy-in editor (`EditHoldingModal`, account detail); each passes the holding's refs (`HoldingResponse`/`PortfolioLine` carry `coingeckoId`/`coinmarketcapId`/`yahooSymbol`) so the collapsed summary lists every linked aggregator. **Registry-wide**: `AssetRegistryModal` — a table of every asset with one column per aggregator (CoinGecko id, CoinMarketCap id, Yahoo symbol) — the column tells at a glance which aggregators can quote the asset, and therefore whether it has a fallback — its status, and its last known EUR value (`lastEurValue`, with a freshness dot) as a quick sanity check that a mapping points at the right coin — shown only for a crypto that actually has a CoinGecko id (an unlinked coin's `lastEurValue` may be a spurious Yahoo-fallback quote on the raw symbol, so it's hidden rather than passed off as validation) — opened from `/accounts` (next to "Add account") and from the admin price-aggregators section; admins confirm an `AUTO` guess in one click (re-pinning every ref it already carries) or expand a row to the same per-aggregator editor. Confirming/editing is gated to crypto rows for now — a product decision, not an engine limit (the editor resolves across every aggregator); surfacing stock/ETF resolution is a later pass. The per-aggregator display metadata (label + verify-URL) lives in one place (`features/assets/aggregators.ts`), shared by the import wizard and the standing editor.
...
- `model/FinancialAsset.java` / `repository/FinancialAssetRepository.java` -- The registry (V80)
- `model/Aggregator.java` / `model/AggregatorSession.java` -- Aggregator identity + encrypted API credentials (V83)
...
| Resolution behind `AssetResolverPort`, one adapter per aggregator | Adding an aggregator = 1 adapter + 1 column + 1 field; the engine/DTOs/frontend loop and don't change | Hardcoding each aggregator in `FinancialAssetService` (what it did through V86: `coinGecko.searchBySymbol`, `setCoingeckoId`, a CoinGecko URL regex) |
