...
- **Yahoo Finance** (`YahooFinancePriceProvider`, `aggregatorKey() = "yahoo"`): the catch-all — `canPrice` is true for any asset carrying a `yahoo_symbol` (read straight off the asset, no fallback) that isn't a plain ISIN, so the router reaches it for whatever CoinGecko couldn't price (stocks, ETFs, indices). An asset with no `yahoo_symbol` — an unresolved crypto, e.g. — is rejected outright: it no longer falls through to a wasted Yahoo call on the raw internal symbol. It queries the `yahoo_symbol` on the unofficial `/v8/finance/chart/{ticker}` endpoint. Fetched per-ticker (no batch). Tickers like `IWDA.AS`, `MC.PA` are already EUR-denominated; foreign-currency tickers (USD/JPY/GBp/...) are converted to EUR inside the adapter via Yahoo's own `{CURRENCY}EUR=X` chart endpoint, with a 15-minute FX cache mirroring the price cache TTL. See [ADR 2026-05-19](../decisions/2026-05-19-yahoo-fx-conversion.md).
...
- `service/FinancialAssetService.java` -- Dynamic symbol resolution (`resolveCrypto`/`getOrCreateStock`), manual mapping, worthless pinning, `previewResolution` (standing candidates)
...
- `FinancialAssetServiceTest` -- resolution rules (dominance, PENDING retry), manual mapping, worthless, purge-and-refetch, `getOrCreateStock` (mint/fill-in/never-clobber-crypto)
- `OpenFigiIsinConverterTest` -- ISIN detection + TR crypto ISINs resolved through the product registry
- `YahooFinancePriceProviderTest` -- unit tests for response parsing, `canPrice` gating on `yahoo_symbol`
