...
- **Resolving at preview costs `/search` calls**: `preview()` resolves every ticker up front (to flag `unresolvedTickers`), so a preview of an unseen CSV fans out one CoinGecko `/search` per new coin — the same work the import would do, just moved earlier. Throttling/back-off for these calls is added in a later change; until then a large first-time import can brush the free-tier rate limit.
