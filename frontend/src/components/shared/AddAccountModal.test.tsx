...
describe('AddAccountModal exchange wizard', () => {
  ...
  function openExchangeWizard() {
    render(<AddAccountModal open onOpenChange={vi.fn()} />)
    // Exchanges sit behind the Crypto tile now — the selector groups exchanges, wallets and the
    // CSV import under one entry rather than listing each on the first screen.
    fireEvent.click(screen.getByText('addAccount.crypto'))
