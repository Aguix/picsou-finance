...
/**
 * The crypto portfolio, rendered inside the Accounts page's Crypto tab (it used to live on a
 * standalone `/crypto` route). Two levels of detail:
 ...
 *
 * The selected view lives in the `?account=` search param (kept alongside `?filter=CRYPTO`) so the
 * import flow can deep-link to a freshly imported account.
 */
export function CryptoPortfolioSection() {
  ...
  function select(account: Account | null) {
    // Merge into the existing params so `?filter=CRYPTO` survives the account switch.
    setSearchParams(
      (prev) => {
        const next = new URLSearchParams(prev)
        if (account) next.set('account', String(account.id))
  ...
  return (
    <div>
      ...
      {/* Global ↔ per-exchange selector, plus the CoinGecko mappings entry point */}
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <Button variant="outline" size="sm" className="ml-auto" onClick={() => setMappingsOpen(true)}>
          <Link2 />
